...
export const clearBox = <T>(b:Box<T>): void => {
    b = [];
}
...
export const makeExtEnv = (vs: string[], vals: Value[], env: Env, callingEnv?: Env): ExtEnv =>{
    let newEnv: ExtEnv,
    name: string = generateEnvId();
    
    if (typeof callingEnv === "undefined")
        newEnv = ({tag: "ExtEnv", frame: makeFrame(vs, vals), env: env, name: name,closures: {},  bodyCircles:{}, callingEnv: env});
    else
        newEnv = ({tag: "ExtEnv", frame: makeFrame(vs, vals), env: env, name: name,closures: {},  bodyCircles:{}, callingEnv: callingEnv});
    persistentEnv[name] = newEnv;
    return newEnv;
}
...
const makeGlobalEnv = (): GlobalEnv => ({tag: "GlobalEnv", frame: makeBox(makeFrame([], [])), closures: {},name: "GE",bodyCircles: {}});
export const clearGlobal = () => {
    theGlobalEnv.closures = {};
    theGlobalEnv.frame = makeBox(makeFrame([], []));
    theGlobalEnv.bodyCircles = {};
}
...
// Assignment 3 part 3 //
type EnvId = string;
export let envIdCounter: Box<number> = makeBox(0);
...
export const clearPersistentEnv = () => {persistentEnv = {};}

export type BodyId = string;  
export let bodyIdCounter: Box<number> = makeBox(0);
