...
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         parseTE, unparseTExp,
         BoolTExp, NumTExp, ProcTExp, StrTExp, TExp, isUnionTExp, isAtomicTExp, UnionTExp, AtomicTExp, makeUnionTExp, isTExp, isTVar } from "./TExp";
...
//          else type<(if test then else)>(tenv) = (t1 | t2)
export const typeofIf = (ifExp: IfExp, tenv: TEnv): TExp | Error => {
    const testTE = typeofExp(ifExp.test, tenv);
    const thenTE: TExp | Error = typeofExp(ifExp.then, tenv);
    const altTE: TExp | Error = typeofExp(ifExp.alt, tenv);
    ...
    if (isError(constraint1))
        return Error(`Incompatible types: ${unparseTExp(testTE)} and ${unparseTExp(makeBoolTExp())} in ${unparse(ifExp)}`);
    else if (isError(constraint2))
        return Error(`Incompatible types: ${unparseTExp(thenTE)} and ${unparseTExp(altTE)} in ${unparse(ifExp)}`);
    else if (constraint2)
        return thenTE;
    else if (isTExp(thenTE) && isTExp(altTE))
        return makeUnionTExp([thenTE,altTE].sort((a:TExp, b:TExp) => {
            return isTVar(a) && isTVar(b) ?
                    a.var < b.var ? -1 : 1 :
                    a.tag < b.tag ? -1 : 
                    1
         }));
    return Error(`cannot happen, logics and sets teory`);
};
...
export const typeofProc = (proc: ProcExp, tenv: TEnv): TExp | Error => {
    ...
    if (isError(constraint1))
        return Error(`Incompatible types: ${unparseTExp(typeofExps(proc.body, extTEnv))} and ${unparseTExp(proc.returnTE)} in ${unparse(proc)}`);
    else
...
// Signature: check-compatible-types?(te1, te2)
// Purpose:   compares two Type Expressions
//            and returns true if te1 is a subtype of te2, false otherwise.
// Example:  checkCompatibleTypes(parseTExp('(Number * (Number -> Boolean | Number) -> Number))',
//                                parseTExp('(Number * (Number -> Boolean) -> Number))') => #t
//           checkCompatibleTypes(parseTExp('((Number | Boolean))',
//                                parseTExp('(Number)' => #t
export const checkCompatibleTypes = (te1: TExp | Error, te2: TExp | Error): boolean | Error => {
    return  isError(te1) ? te1 :
            isError(te2) ? te2 :
            isAtomicTExp(te1) ? isAtomicTExp(te2) ? te1.tag === te2.tag :               // both atomic
                                isUnionTExp(te2) ? unionContainsAtomic(te2,te1) :       // atmoic and union
            false :            // atomic and proc
            isUnionTExp(te1) ? isUnionTExp(te2) ? checkUnionCompatibility(te1,te2) :    // both union
            false :            // union and atomic or proc
            isProcTExp(te1) ? isProcTExp(te2) ? checkProcParamsIdendity(te1.paramTEs, te2.paramTEs) && checkCompatibleTypes(te1.returnTE,te2.returnTE) :
            false : false
}

export const unionContainsAtomic = (te1: UnionTExp, te2: AtomicTExp): boolean | Error => {
    return te1.types.reduce((acc: boolean | Error, curr: TExp) => {
        return  acc ? acc :
            isUnionTExp(curr) ? unionContainsAtomic(curr, te2) :
            isAtomicTExp(curr) ? curr.tag === te2.tag :
            false}, false);
}

export const checkUnionCompatibility = (te1: UnionTExp, te2: UnionTExp): boolean | Error => {
return te1.types.reduce((acc1: boolean | Error, curr1: TExp) =>
    acc1 && te2.types.reduce((acc2: boolean | Error, curr2: TExp) => 
        acc2 || checkCompatibleTypes(curr1, curr2), false)
, true);
}

export const checkProcParamsIdendity = (te1: TExp[], te2: TExp[]): boolean | Error => {
    return te1.length !== te2.length ? false :
    te2.reduce((acc: boolean | Error, curr: TExp, inx) => 
        acc && checkCompatibleTypes(curr, te1[inx]), true);
}
