...
;; [Empty -> void]
*/
import { chain, concat, filter, map, uniq, reduce , sort} from "ramda";
...
export const makeUnionTExp = (exps: TExp[]): UnionTExp => ({tag: "UnionTExp", types: exps});
...
const parseCompoundTExp = (texps: any[]): ProcTExp | UnionTExp | Error => {
    if(texps.indexOf('|') === 1){
        return makeUnionTExp(handleUnion(texps));
    }
    const arrowPos = texps.indexOf('->');
    return (arrowPos === -1)  ? Error(`Procedure type expression without -> - ${texps}`) :
    (arrowPos === 0) ? Error(`No param types in proc texp - ${texps}`) :
    (arrowPos === texps.length - 1) ? Error(`No return type in proc texp - ${texps}`) :
    (texps.slice(arrowPos + 1).indexOf('->') > -1) ? Error(`Only one -> allowed in a procexp - ${texps}`) :
    safeMakeProcTExp(parseTupleTExp(texps.slice(0, arrowPos)),
                     parseTExp(texps[arrowPos + 1]));    
};

const handleUnion = (texps: any[]): TExp[] => {
    const unioned: TExp[] = texps.filter(x => x !== '|').reduce((acc, curr) => acc.concat(parseTExp(curr)), []); 
    const flat = recUnion(unioned);
    const sorted: TExp[] = flat.sort((a:TExp, b:TExp) => {
        return isTVar(a) && isTVar(b) ?
                a.var < b.var ? -1 : 1 :
                a.tag < b.tag ? -1 : 
                1
     }); 
    const sortedNoDup: TExp[] = sorted.filter(function(elem, index, self) {
        if (index === 0) return true;
        const prev: TExp = sorted[index-1];
        return isTVar(prev) && isTVar(elem) ?
            prev.var !== elem.var :
            !equivalentTEs(prev, elem);
    })
    return sortedNoDup;           
}

const recUnion = (types: any[]): any => {
    const flat = types.reduce((acc, curr) => { //fails here
                    if(isUnionTExp(curr)){
                       return acc.concat(recUnion(curr.types));
                    }
                    else{
...
const unparseUnion = (x: UnionTExp): string | Error =>{
    return "(" + x.types.map(type => unparseTExp(type)).join(" | ") + ")";
}
