...
(define take-lzt 
  (lambda (lzt num)
    (do-take-lzt lzt 0 num)
))

(define do-take-lzt
  (lambda (lzt level num)
    (let ((nth-level-children (lzt->nth-level lzt level)))
      (if (>= (children->length nth-level-children) num)
          (first-n-nodes nth-level-children num)
          (if (empty? nth-level-children)
              '()
              (append nth-level-children (do-take-lzt lzt (+ level 1) (- num (children->length nth-level-children))))) 
      )
    )    
  )
)

(define  first-n-nodes
  (lambda (lst num)
    (if (= num 0)
        '()
        (cons (car lst) (first-n-nodes (cdr lst) (- num 1))))
   )
)
(define (children->length children)            
  (if (null? children)                  
      0                            
      (+ 1 (children->length (cdr children))))) 
